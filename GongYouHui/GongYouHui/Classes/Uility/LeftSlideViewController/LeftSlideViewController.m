                     ...
                     andMainView:(UIViewController *)mainVC
{
    if(self = [super init]){
        ...
        self.leftTableview.frame = CGRectMake(0, 0, GYHScreenWidth - kMainPageDistance, GYHScreenHeight);
        //设置左侧tableview的初始位置和缩放系数
        self.leftTableview.transform = CGAffineTransformMakeScale(kLeftScale, kLeftScale);
        self.leftTableview.center = CGPointMake(kLeftCenterX, GYHScreenHeight * 0.5);
...
//    if (((self.mainVC.view.x <= 0) && (_scalef <= 0)) || ((self.mainVC.view.x >= (GYHScreenWidth - kMainPageDistance )) && (_scalef >= 0)))
//    {
//        //边界值管控
//        _scalef = 0;
//        needMoveWithTap = NO;
...
//    
//    //根据视图位置判断是左滑还是右边滑动
//    if (needMoveWithTap && (rec.view.frame.origin.x >= 0) && (rec.view.frame.origin.x <= (GYHScreenWidth - kMainPageDistance)))
//    {
//        CGFloat recCenterX = rec.view.center.x + point.x * self.speedf;
//        if (recCenterX < GYHScreenWidth * 0.5 - 2) {
//            recCenterX = GYHScreenWidth * 0.5;
//        }
//        
...
//        CGFloat scale = 1 - (1 - kMainPageScale) * (rec.view.frame.origin.x / (GYHScreenWidth - kMainPageDistance));
//        
//        rec.view.transform = CGAffineTransformScale(CGAffineTransformIdentity,scale, scale);
//        [rec setTranslation:CGPointMake(0, 0) inView:self.view];
//        
//        CGFloat leftTabCenterX = kLeftCenterX + ((GYHScreenWidth - kMainPageDistance) * 0.5 - kLeftCenterX) * (rec.view.frame.origin.x / (GYHScreenWidth - kMainPageDistance));
//        
//        NSLog(@"%f",leftTabCenterX);
//        
//        
//        //leftScale kLeftScale~1.0
//        CGFloat leftScale = kLeftScale + (1 - kLeftScale) * (rec.view.frame.origin.x / (GYHScreenWidth - kMainPageDistance));
//        
//        self.leftTableview.center = CGPointMake(leftTabCenterX, GYHScreenHeight * 0.5);
//        self.leftTableview.transform = CGAffineTransformScale(CGAffineTransformIdentity, leftScale,leftScale);
//        
//        //tempAlpha kLeftAlpha~0
//        CGFloat tempAlpha = kLeftAlpha - kLeftAlpha * (rec.view.frame.origin.x / (GYHScreenWidth - kMainPageDistance));
...
//            _scalef = 0;
//        }
//        else if (self.mainVC.view.x > (GYHScreenWidth - kMainPageDistance))
//        {
//            [self openLeftView];
//            _scalef = 0;
//        }
...
- (void)closeLeftView
{
    ...
    self.mainVC.view.center = CGPointMake(GYHScreenWidth / 2, GYHScreenHeight / 2);
    self.closed = YES;
...
- (void)openLeftView;
{
    ...
    self.leftTableview.center = CGPointMake((GYHScreenWidth - kMainPageDistance) * 0.5, GYHScreenHeight * 0.5);
